 ...
 *         </p>
 *         <p>
 *         <b>Description: </b>ShoppingListItem is an object that is used to
 *         create/modify a new shopping list item.
 *         </p>
 */
public class ShoppingListItem extends TableRow {
	...
	// Instances of the quantity changer objects. One decreaser and one
	// increaser
	...
	/**
	 * @param context
	 *            The context passed to the ShoppingListItem
	 */
	public ShoppingListItem(Context context) {
		...
		shoppingListEditText.setWidth((int) context.getResources()
				.getDimension(R.dimen.itemSize));
		shoppingListEditText.setHint(context
				.getText(R.string.enter_shopping_list_name));
		shoppingListEditText.setImeOptions(EditorInfo.IME_ACTION_NONE);

		// Add a text changed listener that watches for the items entered into
		// the editText
		...
		// Initialize the decreaseImageButton which is a QuantityChanger and
		// send its value as DECREASER.
		decreaseImageButton = new QuantityChanger(context,
				QuantityChanger.DECREASER);
		// Set the appropriate values for decreaseImageButton QuantityChanger
		decreaseImageButton.setImageDrawable(context.getResources()
		...
		// Initialize the decreaseImageButton which is a QuantityChanger and
		// send its value as INCREASER.
		increaseImageButton = new QuantityChanger(context,
				QuantityChanger.INCREASER);
		...
		increaseImageButton.setImageDrawable(context.getResources()
	...
	private OnClickListener quantityArrowsClickListener = new OnClickListener() {
		public void onClick(View v) {
			// Check the type of QuantityChanger and if its INCREASER, increase
			// the quantity
	 ...
	 * </p>
	 * <p>
	 * <b>Description: </b>QuantityChanger is the imageButton that allows you to
	 * increase/decrease the item's quantity value depending on the
	 * QuantityChanger type.
	 * </p>
	 */
	private class QuantityChanger extends ImageButton {
		...
		/**
		 * @param cont
		 *            The context that uses this object
		 * @param type
		 *            The type of QuantityChanger. INCREASER/DECREASER
		 */
		public QuantityChanger(Context cont, int type) {
			super(cont);
			// Set the type to the one passed.
			this.setType(type);
			// if the type of Quantity changer is neither INCREASER nor
			// DECREASER,
			// throw an Error
			if (type != INCREASER && type != DECREASER) {
				throw new Error(
						"The type of quantity changer is not valid. Make sure to user either Quantity.INCREASER or Quantity.Decreaser");
			} // End of if
		} // End of Constructor
